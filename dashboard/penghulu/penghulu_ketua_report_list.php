...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST["approve"])) {
        ...
        mysqli_query(
            $db,
            "UPDATE ketua_report SET report_status='Approved' WHERE kt_report_id='$kt_report_id'",
        );
        ...
        if (!empty($message)) {
            $report = mysqli_fetch_assoc(
                mysqli_query(
                    $db,
                    "SELECT ketua_id FROM ketua_report WHERE kt_report_id='$kt_report_id'",
                ),
            );
            $ketua_id = $report["ketua_id"];
            mysqli_query(
                $db,
                "INSERT INTO report_messages (kt_report_id, sender_role, receiver_id, message, created_at)
    ...
    if (isset($_POST["reject"])) {
        mysqli_query(
            $db,
            "UPDATE ketua_report SET report_status='Rejected' WHERE kt_report_id='$kt_report_id'",
        );
        header("Location: penghulu_ketua_report_list.php");
        exit();
    }
    ...
    if (isset($_POST["delete"])) {
        mysqli_query(
            $db,
            "DELETE FROM ketua_report WHERE kt_report_id='$kt_report_id'",
        );
        header("Location: penghulu_ketua_report_list.php");
        exit();
    }
